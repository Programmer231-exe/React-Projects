...
export default class DateChecker extends Component {
    constructor(props) {
        ...
        this.restDataSource = new RestDataSource("http://localhost:8082/");
    }
        ...
        <div className="row mt-5">
            <div className="col-md-6 mb-3">
                <div className="form-group">
                    ...
                    <input className={`form-control ${this.state.error.errorCode === "Not Ok" ? "border-danger" : ""}` } name="date" value = {this.state.user.date} onChange={(p) => this.onChange(p)} required = "required"/>
                </div>
                <div className="form-group">
                    <label>Enter the slo</label>
                    <input className={`form-control ${this.state.error.errorCode === "Not Ok" ? "border-danger" : ""}` } name="slo"  value = {this.state.user.slo} onChange={(p) => this.onChange(p)} required = "required"/>
                </div>
                <div className="textcenter">
                    <button type="submit" className="btn btn-primary" onClick={() => this.getResult()}>Post</button>
                </div>
            </div>
            <div className="col-md-6 mb-3">
                ...
                {this.state.error.errorCode && <div className = "alert bg-danger font-weight-bolder">
                    {`Something went wrong ${this.state.error.errorCode}`}
